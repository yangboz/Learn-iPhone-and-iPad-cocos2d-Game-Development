...
-(id) init
{
	if ((self = [super init]))
	{
		...
		score = 10;
		scoreLabel = [CCLabel labelWithString:@"10" fontName:@"Arial" fontSize:40];
...
-(void) updateInfoString
{
	...
	{
		case 0:
			[info setString:@"CCLabel, not updating"];
			scoreLabel.color = ccGREEN;
			break;
		case 1:
			[info setString:@"CCLabel, updating once per second"];
			scoreLabel.color = ccYELLOW;
			break;
		case 2:
			[info setString:@"CCLabel, updating every frame"];
			scoreLabel.color = ccORANGE;
			break;
		case 3:
			[info setString:@"CCLabel, updating 5 times per frame"];
