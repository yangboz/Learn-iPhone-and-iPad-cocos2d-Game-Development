...
-(id) init
{
	if ((self = [super init]))
	{
		...
		CCLabel* label = [CCLabel labelWithString:@"" fontName:@"Marker Felt" fontSize:30];
		...
		CCLabel* label2 = [CCLabel labelWithString:@"tap screen to cycle modes" fontName:@"Marker Felt" fontSize:20];
		label2.position = CGPointMake(size.width / 2, 0);
		label2.anchorPoint = CGPointMake(0.5f, 0);
		[self addChild:label2 z:0 tag:2];

		CCLabel* label3 = [CCLabel labelWithString:@"Note: run this on the device! Simulator performance is irrelevant!" fontName:@"Arial" fontSize:14];
		...
		score = 10;
		bool useBrokenFont = NO;

		if (useBrokenFont == NO)
		{
			scoreLabel = [CCBitmapFontAtlas bitmapFontAtlasWithString:@"10" fntFile:@"arial40.fnt"];
		}
		else
		{
			// flipped-vertically.fnt shows a common bug in Hiero, the png file is vertically flipped
			// set useBrokenFont = YES above to see how this buggy image displays ... the fix is simply to flip
			// the image file manually using an image program
			scoreLabel = [CCBitmapFontAtlas bitmapFontAtlasWithString:@"10" fntFile:@"flipped-vertically.fnt"];
		}
...
-(void) updateInfoString
{
	CCLabel* info = (CCLabel*)[self getChildByTag:1];
	...
	{
		case 0:
			[info setString:@"CCBMFA, not updating"];
			scoreLabel.color = ccGREEN;
			break;
		case 1:
			[info setString:@"CCBMFA, updating once per second"];
			scoreLabel.color = ccYELLOW;
			break;
		case 2:
			[info setString:@"CCBMFA, updating every frame"];
			scoreLabel.color = ccORANGE;
			break;
		case 3:
			[info setString:@"CCBMFA, updating 5 times per frame"];
